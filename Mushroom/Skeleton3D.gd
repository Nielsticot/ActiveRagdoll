...
func _process(delta):
	if Input.is_key_pressed(KEY_SPACE):
		$"Physical Bones/Physical Bone Torso".apply_impulse(Vector3(0, 1, 0))
	if Input.is_key_pressed(KEY_Z):
		$"Physical Bones/Physical Bone Root".apply_impulse(target_skeleton.global_transform.basis.z * 0.2)
		$"Physical Bones/Physical Bone Torso".apply_impulse(target_skeleton.global_transform.basis.y * 0.65)
